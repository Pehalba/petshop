...
class CalendarController {
  ...
  async getDayCount(year, month) {
    ...
    try {
      ...
      const appointments = await this.store.getAppointmentsByDateRange(
        from,
        to
      );
          ...
          pet.vacinas.forEach((vaccine) => {
            if (vaccine.proximaDose) {
              ...
              if (vaccine.proximaDose.includes("T")) {
                ...
                // Se já está no formato YYYY-MM-DD
                const [year, month, day] = vaccine.proximaDose
                  .split("-")
                  .map(Number);
                doseDate = new Date(year, month - 1, day);
              } else {
  ...
  async loadDayAppointments(dateStr) {
    ...
    try {
      ...
      // Buscar agendamentos do dia (filtrar cancelados)
      const allAppointments = await this.store.getAppointmentsByDateRange(
        from,
        to
      );
  ...
  async renderDayList(dateStr, appointments, vaccines = []) {
    ...
    if (appointments.length > 0) {
      titleParts.push(
        `${appointments.length} ${
          appointments.length === 1 ? "serviço" : "serviços"
        }`
      );
    }
  ...
  async renderAppointmentCard(appointment) {
    try {
      ...
      const pet = appointment.petId
        ? await this.store.getPet(appointment.petId)
        : null;

      const time = new Date(appointment.dataHoraInicio).toLocaleTimeString(
        "pt-BR",
        {
          hour: "2-digit",
          minute: "2-digit",
        ...
        <div class="vaccine-card">
          ...
          <div class="vaccine-details">
            ...
            <div class="vaccine-meta">
              ...
              ${
                vaccine.observacoes
                  ? `<span class="vaccine-notes">${vaccine.observacoes}</span>`
                  : ""
              }
            </div>
          </div>
        ...
        <div class="vaccine-card error">
          ...
          <div class="vaccine-details">
            ...
            <div class="vaccine-name">${
              vaccine.nomeVacina || "Vacina desconhecida"
            }</div>
          </div>
        </div>
      `;
